import { Box as MantineBox } from '@mantine/core'
import * as React from 'react'
import { forwardRef } from 'react'
const IconDelete = (props, ref) => {
  return (
    <svg
      width="1em"
      height="1em"
      viewBox="0 0 16 16"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      ref={ref}
      {...props}
    >
      <path
        d="M11.3333 5.99984L7.33333 9.99984M7.33333 5.99984L11.3333 9.99984M1.81333 8.63984L4.69333 12.4798C4.928 12.7927 5.04533 12.9492 5.19403 13.062C5.32574 13.1619 5.47488 13.2365 5.63385 13.2819C5.81334 13.3332 6.00889 13.3332 6.4 13.3332H11.4667C12.5868 13.3332 13.1468 13.3332 13.5746 13.1152C13.951 12.9234 14.2569 12.6175 14.4487 12.2412C14.6667 11.8133 14.6667 11.2533 14.6667 10.1332V5.8665C14.6667 4.7464 14.6667 4.18635 14.4487 3.75852C14.2569 3.3822 13.951 3.07624 13.5746 2.88449C13.1468 2.6665 12.5868 2.6665 11.4667 2.6665H6.4C6.00889 2.6665 5.81334 2.6665 5.63385 2.71776C5.47488 2.76317 5.32574 2.83774 5.19403 2.93767C5.04533 3.05051 4.928 3.20695 4.69333 3.51984L1.81333 7.35984C1.6412 7.58934 1.55514 7.7041 1.52196 7.83013C1.49268 7.94137 1.49268 8.0583 1.52196 8.16955C1.55514 8.29558 1.6412 8.41033 1.81333 8.63984Z"
        stroke="#383E40"
        strokeWidth={1.5}
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </svg>
  )
}
const ForwardRef = forwardRef(IconDelete)
const Delete = forwardRef((props, ref) => {
  if (typeof props.size === 'number') {
    const { size, ...rest } = props
    props = {
      ...rest,
      w: size,
      h: size
    }
  }
  return (
    <MantineBox
      ref={ref}
      {...props}
      component={ForwardRef}
      className={['tiui-icon', 'Delete', props.className].join(' ')}
    />
  )
})
Delete.displayName = 'IconDelete'
export default Delete
